...
export default function NickPromptModal({ suggestedNick, onSubmit, onCancel }: NickPromptModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 select-none animate-menu-backdrop">
      <div className="bg-slate-900/85 backdrop-blur-md rounded-xl p-8 w-full max-w-md mx-4 shadow-floating  animate-menu-pop">
        ...
        <form onSubmit={handleSubmit}>
          <input
            ...
            className="w-full px-4 py-3 rounded-lg bg-surface-1  text-slate-100 text-lg font-mono focus:outline-none focus: transition-colors"
            autoFocus
            maxLength={32}
          />
          ...
          <div className="flex gap-3 mt-6">
            <button
              type="button"
              onClick={onCancel}
              className="flex-1 px-4 py-2 rounded-lg  text-slate-400 hover:bg-surface-2 transition-colors"
            >
              Cancelar
            </button>
            <button
